...
+ 一般来说越靠近顶部梯度越大，收敛较快；越靠近底部梯度越小。收敛较慢；(反向传播过程中，梯度往下一层层累乘，各层的梯度一般是较小的数，越乘越小，因此越靠近底部梯度越小)
...
+ 然后**减去均值**再**除以标准差**再做一个**线性变换(乘一个数再加一个数)**（视频里说的是方差，但是写的是标准差）

$$
x_{i+1}=\gamma \frac{x_i-\mu}{\sigma} + \beta
$$

+ 这里的 $\gamma$ 和 $\beta$ 是可学习的参数，这两个参数的变化范围会被限制（如果均值为0方差为1的分布不是很适合这个神经网络，可以学习出更适合的均值和方差）

(学出的 $\beta$ 就是均值 ${\gamma}$ 就是标准差)
...
**关于上面两行的”作为在特征维“和”作用到通道维“的解释**

1.作用在特征维：全连接层是二维输入，每一行是一个样本，每一列是一个特征，计算并矫正每个特征的均值和方差。举例：输入大小是4x5，那么会计算出5个均值和方差

2.作用在通道维：一张图片有若干个通道，计算并矫正每个通道的均值方差。举例：输入大小是2x3x4x5(批量数，通道数，宽，高)，那么会计算出3个均值和方差
...
两个张量的形状从**最后一维开始向前对其**，对应维度满足

+ 维度大小相等，**或**
+ 其中一个维度大小为**1，或**
+ 其中一个张量在该维度上缺失

则PyTorch会自动将较小的张量“广播”到较大张量的形状

`X`是全连接层的输出，是一个二维张量`(批量大小，像素数)`，有如下操作  
...
`X`是二维，`mean`是一维，但是因为广播机制它们可以相减

# 29.残差网络 ResNet

[视频链接](https://www.bilibili.com/video/BV1bV41177ap?spm_id_from=333.788.videopod.episodes&vd_source=8924ad59b4f62224f165e16aa3d04f00)

## 29.1.残差网络

“假设在卷积神经网络里面只要了解一个神经网络的话只要了解残差网络就行了”

加深神经网络不一定会带来好处。通过使更复杂的模型包含之前的小模型，ResNet保证了网络层数增加(模型变复杂)时，与理论最优解距离不会变长

![29_1](./img/29_1.png)

上图左下角的等式为了跟右边流程图对应或许应该是 $g(x)=x + f(x)$  
...
看代码讲解时发现自己对**批量归一化**没有什么概念，可能是隔太久了，回去看了一下视频和代码，好了现在清晰了。

见`29.2.残差网络代码`

## 29.3.ResNet为什么能训练出1000层的模型

[视频地址](https://www.bilibili.com/video/BV1554y157E3/?spm_id_from=333.788.recommend_more_video.0&vd_source=8924ad59b4f62224f165e16aa3d04f00)

独特的加法运算导致靠近数据层的权重也会有比较大的梯度，更不容易出现梯度消失

# 30.第二部分竞赛：图片分类

[kaggle地址](https://www.kaggle.com/c/classify-leaves)  

[竞赛结果在这里](https://www.bilibili.com/video/BV1Eb4y1C7Fn?spm_id_from=333.788.recommend_more_video.0&trackid=web_related_0.router-related-2206146-cnt78.1763371294586.541&vd_source=8924ad59b4f62224f165e16aa3d04f00)

我自己尝试的结果在git仓库中的位置：`../实战练习/图片分类_李沐`



# 31.深度学习硬件：CPU和GPU

[视频地址](https://www.bilibili.com/video/BV1TU4y1j7Wd?spm_id_from=333.788.videopod.episodes&vd_source=8924ad59b4f62224f165e16aa3d04f00)   

`resnet`适用于图像领域，文本不太行  
...
[视频链接](https://www.bilibili.com/video/BV17y4y1g76q?spm_id_from=333.788.videopod.episodes&vd_source=8924ad59b4f62224f165e16aa3d04f00)

## 36.1.数据增广

数据增广不止能处理图片，还可以处理文本或者语音。  

讲了一个故事：一个基于视觉的产品，通过很多图片数据完成了训练。当把产品放到不同的环境时，不同的灯光，色温，场景会导致功能失效  

如果在训练的时候能模拟部署时会碰到的各种场景可以解决这个问题(增加模型泛化性)
...
增强数据一般是在线生成的，并不会生成一堆增强数据然后存起来；一般的做法是训练的时候读取原始图片然后随机地做增强，测试的时候一般是不会做数据增强的。
...
+ 左右翻转
+ 上下翻转
  + 上下翻转不总是可行(比如楼房不会房顶在下面，而把树叶上下翻转就没什么问题)
